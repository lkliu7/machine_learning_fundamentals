...
def error(w):
    return -np.sum(ll(lr_labels * (data @ w)))

def grad(w):
    p = 1 - l(lr_labels * (data @ w))
    return -(lr_labels * p) @ data
...
def batchGrad(w, vecs, labels):
    """Compute gradient for mini-batch in logistic regression.

    Implements: ∇L = -Σᵢ yᵢ(1 - σ(yᵢxᵢᵀw))xᵢ
